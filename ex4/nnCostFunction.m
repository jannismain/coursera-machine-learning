...
for i = 1:size(X,1)
    y = zeros(10, 1); y(y_int(i)) = 1; y=y';
    y_vec(i, y_int(i)) = 1;
    h = hypothesis(a1(i,:), Theta1, Theta2);
    J = J + (1/m)*sum(-y_vec(i,:).*log(h)-(1-y_vec(i,:)).*log(1-h));
end
...
% 	a3 = sigmoid(z3);
% 
%     y_vec_single = zeros(num_labels, 1); y(y_int(i)) = 1; %y=y';
% 
% 	d3 = a3 - y_vec_single;
% 	d2 = (Theta2' * d3) .* [1; sigmoidGradient(z2)]; d2 = d2(2:end);
%   
